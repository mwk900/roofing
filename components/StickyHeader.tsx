  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-3 py-3 sm:px-4 md:px-6">
        <a
          href="#top"
          className="flex min-h-11 items-center gap-2 rounded-lg px-1 text-navy focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        >
          <span className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-navy text-xs font-bold text-white">
            NR
          </span>
          <span className="text-sm font-semibold leading-tight sm:text-base">
            Northcrest Roofing Nottingham
          </span>
        </a>
        ...
        <nav className="hidden items-center gap-5 text-sm font-medium text-charcoal lg:flex">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="min-h-11 rounded-lg px-2 py-2 transition hover:text-navy focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
            >
              {item.label}
            </a>
          ))}
