...
export default function ContactForm() {
  ...
  if (submitted) {
    return (
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 sm:p-6">
        ...
        <p className="mt-2 text-sm text-slate-700">For this static demo, use one of the actions below to continue your enquiry.</p>
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          ...
          <a href={emailHref} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-navy px-4 text-sm font-semibold text-white">
            Email us
          </a>
          <a
            href={whatsappHref}
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <p className="mt-1 text-sm text-slate-600">Typical response time (demo): within 30–60 minutes during working hours.</p>
      ...
      <button type="submit" className="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-accent px-4 text-sm font-semibold text-white">
        Continue
      </button>
    </form>
  );
